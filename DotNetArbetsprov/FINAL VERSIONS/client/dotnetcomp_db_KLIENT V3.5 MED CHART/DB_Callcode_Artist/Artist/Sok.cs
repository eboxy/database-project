...
namespace DB_Callcode_Artist.Artist
{
    public class Sok
    {
        ...
        HtmlGenericControl display, TextBox text3_Artist, DropDownList text9_VPfr, 
        DropDownList text10_VPtill, DropDownList text11_Ursland, 
        TextBox text12_Ursstad, DropDownList text13_Musiktyp,
        BaseValidator vldArtistOnlyLetters)
        {
            int rowcount = 0;
            ...
            if (!vldArtistOnlyLetters.IsValid)
            {
                //Rensar display från text och gridviews 
                clr.Clean_surfaces_Artist(sida);
                
                string errmess = "<h2>Bara bokstäver får förekomma vid ";
                errmess += "angivelse av ursprungsstad. Försök igen.";
            ...
            else
            {
                ...
                if (ds == null)
                {
                    ...
                    try
                    {
                        ...
                        + ursstad + musiktyp, ds, null, DateTime.Now.AddSeconds(delay),
                        TimeSpan.Zero);
